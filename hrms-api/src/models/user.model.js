...
userSchema.post('save', async function saved (doc, next) {
  try {
    const mailOptions = {
      ...
      html: `<div><h1>Hello new user!</h1><p>Click <a href="${config.hostname}/api/auth/confirm?key=${this.activationKey}">link</a> to activate your new account.</p></div><div></div>`
    }
...
userSchema.statics = {
  ...
  async findAndSendResetLink (payload) {
    ...
    try {
      const mailOptions = {
        ...
        html: `<div><h1>Hello user!</h1><p>Click <a href="${config.hostname}/api/auth/confirm?key=${resetToken}">link</a> Reset Your password.</p></div><div></div>`
      }
      transporter.sendMail(mailOptions, function (error, info) {
        if (error) {
          return null
